...
srcinfo.charges = complex(rand(1,ns));
...
pg = 1;
U1 = cfmm2d(eps,srcinfo,pg);
U2 = c2ddir(srcinfo,stmp,pg);
U2.pot = U2.pottarg;
errs(itest) = norm(real(U1.pot(1:ntest)-U2.pot))/norm(U2.pot);
...
U2.pot = U2.pottarg;
U2.grad = U2.gradtarg;
err = norm(real(U1.pot(1:ntest)-U2.pot))^2+norm(U1.grad(:,1:ntest)-U2.grad)^2;
...
U2 = c2ddir(srcinfo,ttmp,pgt);
errs(itest) = norm(real(U1.pottarg(1:ntest)-U2.pottarg))/norm(U2.pottarg);
...
U1 = cfmm2d(eps,srcinfo,pg,targ,pgt);
U2 = c2ddir(srcinfo,ttmp,pgt);
err = norm(real(U1.pottarg(1:ntest)-U2.pottarg))^2+norm(U1.gradtarg(:,1:ntest)-U2.gradtarg)^2;
ra = norm(U2.pottarg)^2 + norm(U2.gradtarg)^2;
errs(itest) = sqrt(err/ra);
...
U2 = c2ddir(srcinfo,ttmp,pgt);
U2.pot = pottmp;
err = norm(real(U1.pot(1:ntest)-U2.pot))^2 + norm(real(U1.pottarg(1:ntest)-U2.pottarg))^2;
...
U2.grad = gradtmp;
err = norm(real(U1.pot(1:ntest)-U2.pot))^2+norm(U1.grad(:,1:ntest)-U2.grad)^2;
err = err+norm(real(U1.pottarg(1:ntest)-U2.pottarg))^2+norm(U1.gradtarg(:,1:ntest)-U2.gradtarg)^2;
...
srcinfo = rmfield(srcinfo,'charges');
srcinfo = rmfield(srcinfo,'dipstr');
nd = 2;
srcinfo.nd = nd;
srcinfo.charges = complex(rand(nd,ns));
...
U1 = cfmm2d(eps,srcinfo,pg,targ,pgt);
U2 = c2ddir(srcinfo,ttmp,pgt);
tmpp = real(U1.pottarg(:,1:ntest));
tmpp = tmpp(:);
tmpp2 = real(U2.pottarg(:));
errs(itest) = norm(tmpp-tmpp2)/norm(tmpp2);
...
U2 = c2ddir(srcinfo,ttmp,pgt);
U2.pot = pottmp;
p = real(U1.pot(:,1:ntest));
p = p(:);
p2 = real(U2.pot(:));
tp = real(U1.pottarg(:,1:ntest));
tp = tp(:);
tp2 = real(U2.pottarg(:));
