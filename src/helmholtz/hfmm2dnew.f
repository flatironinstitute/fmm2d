...
ccc        allocate(sig2(nd,nsig))        
        ...
        dlam = 1/(dlam/(2*pi))                 
        boxlam = boxsize(ilev)/dlam
ccc        write(6,*) 'before wsave is',(wsave(istart),istart=1,3)
        call zffti(nsig,wsave)
ccc        write(6,*) 'wsave is',(wsave(istart),istart=1,3)
...
ccc                    write(6,*) 'dx,dy',dx,dy
ccc                    write(6,*) 'ix,iy',ix,iy
ccc                    call h2dmplochf(nd,zk,rscales(ilev),
ccc     $                centers(1,jbox),
ccc     1                rmlexp(iaddr(3,jbox)),
ccc     1                nterms(ilev),rscales(ilev),centers(1,ibox),
ccc     3                rmlexp(iaddr(4,ibox)),
ccc     4                nterms(ilev),nsig,wsave,transvecall(1,ix,iy))
...
C$OMP$SCHEDULE(DYNAMIC)
         do ibox = laddr(1,ilev),laddr(2,ilev)
            ...
            iend = iexpcse(2,ibox)
            npts = iend - istart + 1
ccc            call h2d_mptosig(nd,nterms(ilev),nsig,
ccc     1           rmlexp(iaddr(2,ibox)),sig,wsave)
            ...
            if(npts.gt.0) then
               do i=1,nchild
                  ...
                  if (boxlam .gt. 16.0d0) then
                   call h2dmpmphf2(nd,zk,rscales(ilev),
