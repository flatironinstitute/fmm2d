...
C
C WSAVE modified to only store exponentials - memory allocated in
C       subsequent uses of wsave to make it thread safe...  
C 1/28/2023  - Leslie Greengard
C
C 
C ******************************************************************
C 
...
C 
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      double precision, allocatable :: wsavep(:)
      IF (N .EQ. 1) RETURN
      allocate(wsavep(4*n+100))
      do i = 1,4*n+100
         wsavep(i) = wsave(i)
      enddo
      IW1 = N+N+1
      IW2 = IW1+N+N
      CALL ZFFTB1 (N,C,WSAVEP,WSAVEP(IW1),WSAVEP(IW2))
      RETURN
      END
  ...
  117 CONTINUE
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      IW1 = N+N+1
      IW2 = IW1+N+N
      CALL ZFFTF1 (N,C,WSAVEP,WSAVEP(IW1),WSAVEP(IW2))
      RETURN
      END
...
C
  ...
  103 DO 104 I=1,N
         WSAVEP(I) = R(I)
  104 CONTINUE
      CALL DFFTF (N,WSAVEP,WSAVEP(N+1))
      CF = 2.0D0/DBLE(N)
      CFM = -CF
      AZERO = .5D0*CF*WSAVEP(1)
      NS2 = (N+1)/2
      NS2M = NS2-1
      DO 105 I=1,NS2M
         A(I) = CF*WSAVEP(2*I)
         B(I) = CFM*WSAVEP(2*I+1)
  105 CONTINUE
      IF (MOD(N,2) .EQ. 1) RETURN
      A(NS2) = .5D0*CF*WSAVEP(N)
      B(NS2) = 0.0D0
      RETURN
      END
...
C
  ...
  144 CONTINUE
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      enddo
      IF (N .EQ. 1) RETURN
      CALL DFFTB1 (N,R,WSAVEP,WSAVEP(N+1),WSAVEP(2*N+1))
      RETURN
      END
  ...
  117 CONTINUE
	...
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      ...
      enddo
      IF (N .EQ. 1) RETURN
      CALL DFFTF1 (N,R,WSAVEP,WSAVEP(N+1),WSAVEP(2*N+1))
      RETURN
      END
