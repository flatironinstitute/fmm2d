...
Set-Variable unpacked_wheel -Value (get-item .\tpm\fmm2dpy-*).FullName
Copy-Item -Path C:\msys64\mingw64\bin\libgcc_s_seh-*.dll -Destination ([IO.Path]::Combine($unpacked_wheel, 'fmm2dpy'))
Copy-Item -Path C:\msys64\mingw64\bin\libgomp-*.dll -Destination ([IO.Path]::Combine($unpacked_wheel, 'fmm2dpy'))
Copy-Item -Path C:\msys64\mingw64\bin\libgfortran-*.dll -Destination ([IO.Path]::Combine($unpacked_wheel, 'fmm2dpy'))
Copy-Item -Path C:\msys64\mingw64\bin\libwinpthread-*.dll -Destination ([IO.Path]::Combine($unpacked_wheel, 'fmm2dpy'))
Copy-Item -Path C:\msys64\mingw64\bin\libquadmath-*.dll -Destination ([IO.Path]::Combine($unpacked_wheel, 'fmm2dpy'))
