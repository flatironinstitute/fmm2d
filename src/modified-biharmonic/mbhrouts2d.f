...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
c     global variables
      integer ns, nterms, ifcharge, ifdipole, ifquad, ifoct
      real *8 beta, source(2,*), charge(nd,*), dipstr(nd,*)
      real *8 dipvec(nd,2,*)
...
c     
     ...
     1              quadstr(j,i)*(quadvec(j,1,iii) +
     1              quadvec(j,3,iii))/(4.0d0*pi)
            enddo
         endif
...
c     global
      ...
      real *8 pot(nd,ntarg)
      integer nterms, ntarg, nd, ifgrad, ifhess
c     local
      ...
      allocate(mptemp1(0:nterms+2),mptemp2(0:nterms+2),
     1     diffs(0:nterms+2),allocate(kvec(0:nterms+2)))
...
c
     ...
     1     mbhmpoleyy(nd,0:nterms+2))
      ...
      do j = 1,nd
         ympolexx(j,1) = -beta/1.0d0/rscale*dreal(j,ympolex(j,0))
         ympolexy(j,1) = -beta/1.0d0/rscale*dreal(j,ympolex(j,0))*eye
         ympoleyy(j,1) = -beta/1.0d0/rscale*dreal(j,ympoley(j,0))*eye

         mbhmpolexx(j,1) = -beta/1.0d0/rscale*dreal(j,mbhmpolex(j,0))
         mbhmpolexy(j,1) = 
     1        -beta/1.0d0/rscale*dreal(j,mbhmpolex(j,0))*eye
         mbhmpoleyy(j,1) = 
     1        -beta/1.0d0/rscale*dreal(j,mbhmpoley(j,0))*eye
      enddo
...
c     local
     ...
     1     mbhlocxx,mbhlocxy,mbhlocyy,llocx,llocy,llocxx,llocxy,llocyy
      ...
      allocate(pow(0:nterms+2),dpow(0:nterms+2))
      allocate(mptemp1(0:nterms+2),mptemp2(0:nterms+2))
      allocate(diffs(0:nterms+2))
...
c     get values of difference functions
     ...
     1              dimag(mptemp1(l))*dimag(mbhloc(j))
               pot(j,i) = pot(j,i) +
...
c     get values of difference functions
     ...
     1           dreal(mptemp2(0))*dreal(llocy(j,0))
         ...
         do l = 1,nterms+1
            do j = 1,nd
               grad(j,1,i) = grad(j,1,i) +
     1              dreal(mptemp1(j))*dreal(mbhlocx(j,l)) +
     1              dimag(mptemp1(j))*dimag(mbhlocx(j,l))
               grad(j,1,i) = grad(j,1,i) +
     1              dreal(mptemp2(j))*dreal(llocx(j,l)) +
     1              dimag(mptemp2(j))*dimag(llocx(j,l))
               grad(j,2,i) = grad(j,2,i) +
     1              dreal(mptemp1(j))*dreal(mbhlocy(j,l)) +
     1              dimag(mptemp1(j))*dimag(mbhlocy(j,l))
               grad(j,2,i) = grad(j,2,i) +
     1              dreal(mptemp2(j))*dreal(llocy(j,l)) +
     1              dimag(mptemp2(j))*dimag(llocy(j,l))
            enddo
         enddo
      enddo
...
c     local
     ...
     1           beta/2*(eye)*rscale*mbhloc(j,i)*dtemp
      ...
      enddo
c
      allocate(mbhlocxx(nd,0:nterms+1),mbhlocxy(nd,0:nterms+1),
     1     mbhlocyy(nd,0:nterms+1),
     1     llocxx(nd,0:nterms+1),llocxy(nd,0:nterms+1),
     1     llocyy(nd,0:nterms+1))
...
c     get values of difference functions
     ...
     1           + dreal(mptemp2(0))*dreal(llocyy(j,0))
         ...
         do l = 1,nterms+1
            do j = 1,nd
               hess(j,1,i) = hess(j,1,i) +
     1              dreal(mptemp1(j))*dreal(mbhlocxx(j,l)) +
     1              dimag(mptemp1(j))*dimag(mbhlocxx(j,l))
               hess(j,1,i) = hess(j,1,i) +
     1              dreal(mptemp2(j))*dreal(llocxx(j,l)) +
     1              dimag(mptemp2(j))*dimag(llocxx(j,l))
               hess(j,2,i) = hess(j,2,i) +
     1              dreal(mptemp1(j))*dreal(mbhlocxy(j,l)) +
     1              dimag(mptemp1(j))*dimag(mbhlocxy(j,l))
               hess(j,2,i) = hess(j,2,i) +
     1              dreal(mptemp2(j))*dreal(llocxy(j,l)) +
     1              dimag(mptemp2(j))*dimag(llocxy(j,l))
               hess(j,3,i) = hess(j,3,i) +
     1              dreal(mptemp1(j))*dreal(mbhlocyy(j,l)) +
     1              dimag(mptemp1(j))*dimag(mbhlocyy(j,l))
               hess(j,3,i) = hess(j,3,i) +
     1              dreal(mptemp2(j))*dreal(llocyy(j,l)) +
     1              dimag(mptemp2(j))*dimag(llocyy(j,l))
            enddo
         enddo
      enddo
...
c     local variables
     ...
     1        mbhmpolesrc(1,0,i),ympolesrc(1,0,i),ntermsrc,
     2        rscale,center,mbhloc,lloc,nterms,carray,ldc)
      enddo
...
c     global
      ...
      real *8 rscale2, center2(2), carray(0:ldc,0:ldc)
...
c     local
      ...
      allocate(jtemp(0:nterms+3),difftemp(0:nterms+3),
     1     powtemp(0:nterms+3),
     2     diffs(0:nterms+3),ival(0:nterms+3),pow(0:nterms+3),
     3     dpow(0:nterms+3))
...
c     local
     ...
     1     dfac2(:)
      ...
      allocate(diffs(0:nterms+5),kvec(0:nterms+5),pow(0:nterms+5),
     1     dpow(0:nterms+5),dfac2(0:nterms+5),ktemp(0:nterms+5),
     2     difftemp(0:nterms+5),powtemp(0:nterms+5))
...
c     
      ...
      do i = 1,nterms2
         do l = 1,nd
            mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1           2.0d0*(dreal(mbhmpole(l,0))*dreal(ktemp(i)) +
     2           ima*dreal(mbhmpole(l,0))*dimag(ktemp(i)))
            mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1           2.0d0*(dreal(ympole(l,0))*dreal(ktemp(i))
     2           +ima*dreal(ympole(l,0))*dimag(ktemp(i)))
            lloc(l,i) = lloc(l,i) + isign*rsi5*
     1           2.0d0*(dreal(mbhmpole(l,0))*dreal(difftemp(i))
     2           + ima*dreal(mbhmpole(l,0))*dimag(difftemp(i)))*dfac2(i)
            lloc(l,i) = lloc(l,i) + isign*rsi5*
     1           2.0d0*(dreal(ympole(l,0))*dreal(ktemp(i))
     2           +ima*dreal(ympole(l,0))*dimag(ktemp(i)))*dfac2(i)
         enddo
         rsj=rscale1
         rsj2=rscale1**2
         do j = 1,min(nterms1,i)
            do l = 1,nd
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(ktemp(i-j))
     1              -dimag(mbhmpole(l,j))*dimag(ktemp(i-j))
     2              +ima*(dreal(mbhmpole(l,j))*dimag(ktemp(i-j))
     3              +dimag(mbhmpole(l,j))*dreal(ktemp(i-j))))*rsj2
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(ktemp(i+j))
     1              +dimag(mbhmpole(l,j))*dimag(ktemp(i+j))
     2              +ima*(dreal(mbhmpole(l,j))*dimag(ktemp(i+j))
     3              -dimag(mbhmpole(l,j))*dreal(ktemp(i+j))))
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(ympole(l,j))*dreal(ktemp(i-j))
     1              -dimag(ympole(l,j))*dimag(ktemp(i-j))
     2              +ima*(dreal(ympole(l,j))*dimag(ktemp(i-j))
     3              +dimag(ympole(l,j))*dreal(ktemp(i-j))))*rsj2
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(ympole(l,j))*dreal(ktemp(i+j))
     1              +dimag(ympole(l,j))*dimag(ktemp(i+j))
     2              +ima*(dreal(ympole(l,j))*dimag(ktemp(i+j))
     3              -dimag(ympole(l,j))*dreal(ktemp(i+j))))
               lloc(l,i) = lloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(ktemp(i-j))
     1              -dimag(mbhmpole(l,j))*dimag(ktemp(i-j))
     2              +ima*(dreal(mbhmpole(l,j))*dimag(ktemp(i-j))
     ...
     5              dfac2(i)
               lloc(l,i) = lloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(difftemp(i+j))
     1              +dimag(mbhmpole(l,j))*dimag(difftemp(i+j))
     2              +ima*(dreal(mbhmpole(l,j))*dimag(difftemp(i+j))
     3              -dimag(mbhmpole(l,j))*dreal(difftemp(i+j))))*
     5              dfac2(i)
               lloc(l,i) = lloc(l,i) + isign*rsi5*
     1              (dreal(ympole(l,j))*dreal(ktemp(i-j))
     1              -dimag(ympole(l,j))*dimag(ktemp(i-j))
     ...
     1              +dimag(ympole(l,j))*dimag(ktemp(i+j))
     2              +ima*(dreal(ympole(l,j))*dimag(ktemp(i+j))
     3              -dimag(ympole(l,j))*dreal(ktemp(i+j))))*dfac2(i)
            enddo
            rsj=rsj*rscale1
            rsj2=rsj2*rscale1**2
         enddo
         do j = i+1,nterms1
            do l = 1,nd
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(ktemp(j-i))
     1              +dimag(mbhmpole(l,j))*dimag(ktemp(j-i))
     2              -ima*(dreal(mbhmpole(l,j))*dimag(ktemp(j-i))
     3              -dimag(mbhmpole(l,j))*dreal(ktemp(j-i))))*rsi2
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(l,j))*dreal(ktemp(i+j))
     1              +dimag(mbhmpole(l,j))*dimag(ktemp(i+j))
     2              +ima*(dreal(mbhmpole(l,j))*dimag(ktemp(i+j))
     3              -dimag(mbhmpole(l,j))*dreal(ktemp(i+j))))
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(ympole(l,j))*dreal(ktemp(j-i))
     1              +dimag(ympole(l,j))*dimag(ktemp(j-i))
     2              -ima*(dreal(ympole(l,j))*dimag(ktemp(j-i))
     3              -dimag(ympole(l,j))*dreal(ktemp(j-i))))*rsi2
               mbhloc(l,i) = mbhloc(l,i) + isign*rsi5*
     1              (dreal(ympole(l,j))*dreal(ktemp(i+j))
     1              +dimag(ympole(l,j))*dimag(ktemp(i+j))
     2              +ima*(dreal(ympole(l,j))*dimag(ktemp(i+j))
     3              -dimag(ympole(l,j))*dreal(ktemp(i+j))))
               lloc(l,i) = lloc(l,i) + isign*rsi5*
     1              (dreal(mbhmpole(j))*dreal(ktemp(j-i))
     1              +dimag(mbhmpole(l,j))*dimag(ktemp(j-i))
     2              -ima*(dreal(mbhmpole(l,j))*dimag(ktemp(j-i))
     3              -dimag(mbhmpole(l,j))*dreal(ktemp(j-i))))*rsi2*
     5              dfac2(i)
...
c     local
      ...
      complex *16, allocatble :: jtemp(:), difftemp(:), powtemp(:)
...
c
      ...
      allocate(jtemp(0:nterms+5),difftemp(0:nterms+5),
     1     powtemp(0:nterms+5),ival(0:nterms+5),diffs(0:nterms+5),
     1     pow(0:nterms+5),dpow(0:nterms+5),pow2(0:nterms+5),
     1     dfac2(0:nterms+5))
